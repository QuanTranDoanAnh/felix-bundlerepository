...
public class SpecXMLPullParser
{
    ...
    private static Requirement parseRequirement(XmlPullParser reader) throws Exception
    {
        ...
        System.out.println("Filter directives is: " + directives.toString());
        
        if (directives.get(Namespace.REQUIREMENT_FILTER_DIRECTIVE) == null) {
        	return null;
        }
        System.out.println("Filter directives before: " + directives.get(Namespace.REQUIREMENT_FILTER_DIRECTIVE).toString());
        String filter = directives.remove(Namespace.REQUIREMENT_FILTER_DIRECTIVE);
        System.out.println("Filter directives after: " + filter);
